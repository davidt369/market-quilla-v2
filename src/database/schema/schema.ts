...
// --- Enums ---
export const estadoPagoEnum = pgEnum('estado_pago_enum', ['pendiente', 'pagado', 'parcial', 'anulado']);
export const estadoPaqueteEnum = pgEnum('estado_paquete_enum', ['registrado', 'en_almacen', 'entregado', 'devuelto', 'perdido']);
export const rolBaseEnum = pgEnum('rol_base_enum', ['administrador', 'supervisor', 'recepcionista', 'cajero']);
export const tipoClienteEnum = pgEnum('tipo_cliente_enum', ['persona', 'empresa']);
// Nota: 'qr' está perfecto para integraciones de pago rápido locales
export const metodoPagoEnum = pgEnum('metodo_pago_enum', ['efectivo', 'qr', 'transferencia', 'tarjeta']);
export const tipoMovimientoEnum = pgEnum('tipo_movimiento_enum', ['ingreso', 'egreso']);
export const momentoPagoEnum = pgEnum('momento_pago_enum', ['al_registrar', 'al_entregar']);

// --- Tablas ---

export const tbusuarios = pgTable('tbusuarios', {
  pk_id_usuario: integer('pk_id_usuario').primaryKey().generatedAlwaysAsIdentity(),
...
export const tbclientes = pgTable('tbclientes', {
  pk_id_cliente: integer('pk_id_cliente').primaryKey().generatedAlwaysAsIdentity(),
...
export const tbpaquetes = pgTable('tbpaquetes', {
  pk_id_paquete: integer('pk_id_paquete').primaryKey().generatedAlwaysAsIdentity(),
  fk_id_remitente: integer('fk_id_remitente').notNull().references(() => tbclientes.pk_id_cliente, { onDelete: 'restrict' }),
  fk_id_destinatario: integer('fk_id_destinatario').notNull().references(() => tbclientes.pk_id_cliente, { onDelete: 'restrict' }),
  fk_id_usuario: integer('fk_id_usuario').references(() => tbusuarios.pk_id_usuario, { onDelete: 'set null' }),
  ...
  valorDeclarado: numeric('valor_declarado', { precision: 10, scale: 2 }).default('0'),
  ...
  estadoPaquete: estadoPaqueteEnum('estado_paquete').default('registrado').notNull(),
  ...
  fechaHoraRegistro: timestamp('fecha_hora_registro', { withTimezone: true }).defaultNow().notNull(),
...
export const tbcajaTurnos = pgTable('tbcaja_turnos', {
  pk_id_cajaTurno: integer('pk_id_cajaTurno').primaryKey().generatedAlwaysAsIdentity(),
  fk_id_usuario: integer('fk_id_usuario').references(() => tbusuarios.pk_id_usuario, { onDelete: 'restrict' }),
  ...
  montoInicial: numeric('monto_inicial', { precision: 10, scale: 2 }).default('0'),
...
export const tbcajaMovimientos = pgTable('tbcaja_movimientos', {
  pk_id_movimiento: integer('pk_id_movimiento').primaryKey().generatedAlwaysAsIdentity(),
  fk_id_cajaTurno: integer('fk_id_cajaTurno').notNull().references(() => tbcajaTurnos.pk_id_cajaTurno, { onDelete: 'restrict' }),
  fk_id_usuario: integer('fk_id_usuario').references(() => tbusuarios.pk_id_usuario, { onDelete: 'set null' }),
  fk_id_paquete: integer('fk_id_paquete').references(() => tbpaquetes.pk_id_paquete, { onDelete: 'set null' }),
...
export const tbauditoria = pgTable('tbauditoria', {
  pk_id_auditoria: integer('pk_id_auditoria').primaryKey().generatedAlwaysAsIdentity(),
  fk_id_usuario: integer('fk_id_usuario').references(() => tbusuarios.pk_id_usuario, { onDelete: 'set null' }),
