...
export async function GET(req: Request) {
  try {
    ...
    if (!session || !session.user.empresaId) {
      return NextResponse.json({ error: "No autorizado. Falta contexto de empresa en la sesión." }, { status: 401 });
    }

    const { searchParams } = new URL(req.url);
    const sucursalIdParam = searchParams.get("sucursalId");

    let condition = eq(usuarios.empresaId, session.user.empresaId);

    if (sucursalIdParam) {
       condition = and(condition, eq(usuarios.sucursalId, parseInt(sucursalIdParam))) as any;
    }

    // Obtener usuarios de la empresa actual
    const empresaUsuarios = await db.select({
        id: usuarios.id,
...
export async function POST(req: Request) {
  try {
    ...
    if (!session || !session.user.empresaId || !session.user.sucursalId) {
      return NextResponse.json({ error: "No autorizado. Su sesión no tiene empresa o sucursal asociada." }, { status: 401 });
    }
    ...
    // Validación básica
    if (!nombreCompleto || !nombreUsuario || !password || !rolBase) {
      return NextResponse.json({ error: "Faltan campos obligatorios (nombreCompleto, nombreUsuario, password, rolBase)" }, { status: 400 });
    }
    ...
    if (error.code === '23505') { // Unique constraint violation (ej: nombre de usuario duplicado)
