...
import { auditable, db } from "@/database";
import {
  tbcajaMovimientos,
  tbcajaTurnos,
  tbclientes,
...
export function handleDbErrorPaquete(error: any): never {
  ...
  if (error.code === "23503") {
    throw new Error(
      "No se pudo completar la operación: datos relacionados no encontrados (ej. remitente, destinatario o usuario).",
    );
  }
  throw new Error(
    error.message || "Error interno al procesar la operación con paquetes.",
  );
}
...
export const createPaquete = auditable(async (tx, data: PaqueteInsert) => {
  try {
    if (data.ubicacionAlmacen) {
      ...
      if (paqueteExistente) {
        throw new Error(
          "El paquete ya está registrado con esa ubicación de almacén.",
        );
      }
    }
    ...
    const [newPaquete] = await tx.insert(tbpaquetes).values(data).returning({
...
export const createPaqueteCompletoTransaction = auditable(
  async (tx, data: PaqueteCompletoFormData, usuarioId: number) => {
    try {
      ...
      if (!turnoActivo) {
        throw new Error(
          "Debe tener una caja abierta para poder registrar paquetes.",
        );
      }
      ...
      if (destinatarioId) {
        ...
        const [newDestinatario] = await tx
          .insert(tbclientes)
          .values({
            nombre_completo: data.destinatario.nombre_completo,
            ci_o_cel: data.destinatario.ci_o_cel,
      ...
      // 3. Validar Paquete
      const finalUbicacion = data.ubicacionAlmacen;
      let isAutoBox = false;
      ...
      if (finalUbicacion && finalUbicacion.includes("/AUTO/")) {
        ...
        tempUbicacion = finalUbicacion.replace(
          "/AUTO/",
        ...
        const paqueteExistente = await tx.query.tbpaquetes.findFirst({
          where: (p, { eq, and, isNull }) =>
            and(eq(p.ubicacionAlmacen, finalUbicacion!), isNull(p.deletedAt)),
          columns: { pk_id_paquete: true },
        });
        if (paqueteExistente) {
          throw new Error(
      ...
      if (isAutoBox && finalUbicacion) {
        const realUbicacion = finalUbicacion.replace(
          "/AUTO/",
      ...
      if (data.momentoPago === "al_registrar") {
        if (!data.metodoPago) {
          throw new Error(
            "Se requiere especificar un método de pago al registrar un paquete pagado al instante.",
          );
        }

        const montoCobrado =
          data.diasOferta && data.diasOferta > 0 && data.precioOferta != null
...
export const updatePaqueteCompletoTransaction = auditable(
  async (tx, id: number, data: PaqueteCompletoFormData, usuarioId: number) => {
    try {
      ...
      if (current.estadoPaquete === "entregado") {
        throw new Error(
          "El paquete ya fue entregado y no puede ser modificado.",
        );
      }
      ...
      if (
        ...
        data.ubicacionAlmacen !== current.ubicacionAlmacen
      ) {
        const paqueteExistente = await tx.query.tbpaquetes.findFirst({
          where: (p, { eq, and, isNull }) =>
            and(
        ...
        if (paqueteExistente && paqueteExistente.pk_id_paquete !== id) {
          throw new Error(
            "El paquete no puede ser actualizado porque ya existe otro con esa ubicación de almacén.",
          );
        }
      }
      ...
      const [paquete] = await tx
        ...
        .set(updateData)
        .where(
          and(eq(tbpaquetes.pk_id_paquete, id), isNull(tbpaquetes.deletedAt)),
        )
        .returning();
  ...
  try {
    ...
    if (q) {
      ...
      const qFilters = [
        ...
        exists(
          db
            .select({ id: tbclientes.pk_id_cliente })
            .from(tbclientes)
            .where(
              and(
      ...
      if (searchId !== null) {
        qFilters.push(eq(tbpaquetes.pk_id_paquete, searchId));
      }
    ...
    const data = await db.query.tbpaquetes.findMany({
      ...
      with: {
        ...
        usuarioEntrega: { columns: { nombre_completo: true } },
        movimientosCaja: {
          columns: { metodoPago: true, descripcion: true, monto: true },
        },
      },
    });
...
export async function getPaqueteById(id: number) {
  try {
    const paquete = await db.query.tbpaquetes.findFirst({
      where: and(
        eq(tbpaquetes.pk_id_paquete, id),
...
export const updatePaquete = auditable(
  async (tx, id: number, data: PaqueteUpdate) => {
    try {
      const current = await getPaqueteById(id);
      ...
      if (data.momentoPago) {
        ...
        data.momentoPago =
          data.estadoPago === "pagado" ? "al_registrar" : "al_entregar";
      }
      ...
      if (!updated) {
        throw new Error(
          `No se pudo actualizar. El paquete con ID ${id} no fue encontrado.`,
        );
      }
...
export const deletePaquete = auditable(async (tx, id: number) => {
  try {
    ...
    if (!deleted) {
      throw new Error(
        `No se pudo eliminar. El paquete con ID ${id} no existe o ya estaba eliminado.`,
      );
    }

    return {
      message: "Paquete eliminado exitosamente.",
      id: deleted.pk_id_paquete,
    };
  } catch (error: any) {
    handleDbErrorPaquete(error);
  }
